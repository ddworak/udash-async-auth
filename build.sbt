...
val commonSettings = noPublishSettings ++ Seq(
  moduleName := "my-udash-application-" + moduleName.value,
...
lazy val frontend = project.in(file("frontend"))
  ...
  .settings(
    ...
    compileCss := Def.taskDyn {
      ...
      dir.mkdirs()
      (backend / Compile / runMain).toTask(s" io.company.app.backend.css.CssRenderer $path false")
    }.value,
...
lazy val backend = project.in(file("backend"))
  ...
  .settings(
    ...
    Compile / mainClass := Some("io.company.app.backend.Launcher"),
  )
...
lazy val packager = project
  ...
  .settings(commonSettings)
  .settings(
    normalizedName := "my-udash-application",
